...
class BackgroundUploadExtension: PHBackgroundResourceUploadExtension {
    ...
    /// This method checks multiple sources to determine if an asset needs to be uploaded:
    /// 1. uploaded_assets table - assets that have been successfully uploaded by this extension
    /// 2. assets_on_server table - assets that are already on the Immich server (synced from main app's SQLite import)
    /// 3. hash_checked table - assets whose hash has been checked against the server
    /// 4. Live server check - verify with Immich server using SHA1 hash if not in local cache
    private func getUnprocessedResources(from library: PHPhotoLibrary) -> [PHAssetResource] {
        ...
        fetchOptions.fetchLimit = 100  // Process up to 100 at a time
        
        let assets = PHAsset.fetchAssets(with: fetchOptions)
        
        log("Fetched \(assets.count) assets from photo library")
        
        // Collect candidate assets that need hash checking
        ...
        assets.enumerateObjects { (asset, _, stop) in
            ...
            // Limit candidate count for processing
            if candidateAssets.count >= 50 {
                stop.pointee = true
            }
        }
        
        log("Found \(candidateAssets.count) candidate assets to check")
        
        // Check each candidate asset against the server
        for asset in candidateAssets {
            ...
            guard let primaryResource = getPrimaryResource(from: resources) else {
                continue
            }
            ...
            for resource in resources {
                if self.shouldUploadResource(resource) {
                    ...
                    if !self.database.isResourceUploaded(
                        ...
                        unprocessedResources.append(resource)
                    }
                }
            }
            
            // Limit total resources to process
            if unprocessedResources.count >= 50 {
                break
            }
        }
        
        log("Found \(unprocessedResources.count) unprocessed resources to upload")
    ...
    private func buildDestination(for resource: PHAssetResource) -> URLRequest? {
        ...
        request.setValue("ios-immich-uploader", forHTTPHeaderField: "X-Device-Id")
