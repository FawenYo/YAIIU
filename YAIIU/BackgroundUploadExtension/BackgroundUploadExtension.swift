...
final class BackgroundUploadExtension: PHBackgroundResourceUploadExtension {
    ...
    private func acknowledgeCompletedJobs() throws {
        ...
        for i in 0..<jobs.count where !isCancelled {
            ...
            database.recordUploadedAsset(
                ...
                filename: resolveOriginalFilename(for: resource),
                immichId: "unknown",
                fileSize: 0,
                isDuplicate: false
            )
    ...
    private func createNewUploadJobs() throws {
        ...
        try library.performChangesAndWait {
            for resource in resources where !self.isCancelled {
                ...
                let resolvedFilename = self.resolveOriginalFilename(for: resource)
    ...
    private func buildDestination(for resource: PHAssetResource) -> URLRequest?
    {
        ...
        let resolvedFilename = resolveOriginalFilename(for: resource)
        guard let asset = fetchAsset(for: resource) else {
            logWarning("Could not fetch asset for resource: \(resolvedFilename), skipping upload")
            return nil
        }
    ...
    /// Resolves "FullSizeRender" filenames (generated by iOS when a photo is edited)
    /// back to the original photo resource's filename.
    private func resolveOriginalFilename(for resource: PHAssetResource) -> String {
